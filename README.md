...
可以自行下载安装包，学校也提供 MATLAB 2018 正版软件免费下载（但使用在线功能需要在 DLUT 教育网下）。具体安装步骤移步百度，都有十分详细的教程。安装时间比较长，建议在校园网环境下安装。（[学校超算中心网址](https://hpc.dlut.edu.cn/)）
...
简单来说，LaTeX 是用于生成 PDF 文档的编辑与排版软件，排版效果、对复杂公式的支持与对插图的支持相比 Word 更优秀。

LaTeX 的祖宗是 TeX，提到 TeX 就不得不说起它的创始人——高德纳教授，计算机学界泰斗地位。当时，计算机还有几个房间这么大的时候，他在写他的《计算机程序设计艺术》（*TAOCP*）。第一卷是 1968 年第一次出版的。当他已经写完了三卷，给第一卷与第二卷出了第二版后，由于这时候刚刚使用照排技术（之前还用的是铅字），技术不成熟，发现他的书籍排得实在太丑了，尤其是数学公式，于是他作出决定，自己写一个排版引擎出来，这个时候是 1977 年，次年他就发布了第一个版本。一个比较重要的版本是 TeX82，在 1982 年发布的，这次重写了整个引擎，使用了一种新的断行算法等等。最后在 1989 年正式使用 TeX 来排版书籍，顺便还出了一本书 *The TeXbook*。在那个年代，1970 年到 1990 年，流行的编程语言还是 Pascal，而就在 1978 年，刚刚才有了 K&R 的那本圣经，ANSI C 是 1989 年的事情。

虽然看 TeX 出身如此牛逼，但是，真正能把 *The TeXbook* 啃下来，然后排出自己书籍的人，可谓是少数（当时有电脑的都是少数）。当然，这期间还有一些重要的参考书籍出版比如 *TeX for the Impatient*。不过一般人可能只是想在自己的书里面插入数学公式，画一个表格，插入图片（在那个年代插图可是一件极其费劲的事情，想想 JPEG 标准出来的时候是 1992 年，当年的格式都是 PostScript）。

LaTeX 是 Leslie Lamport 在 1983 年为了用 TeX 写一本书而开始的。他觉得 TeX 的语法格式太复杂了，可以化简，于是就有了 LaTeX。等到 LaTeX 写完了，那本书就不想写了。

可以这么说，LaTeX 让所有人都可以去当调包侠，而在大部分情况下不用考虑底层的排版问题。

LaTeX 作为一个标记语言，它的核心思想在于，通过 `\section` 和 `\paragraph` 等语句，规定了每一句话在文章中所从属的层次，从而极大方便了对各个层次批量处理。而 LaTeX 作为一个排版引擎也在努力赶上现代排版技术（比如 OpenType 字体技术、PDF 内嵌 Javascript 与媒体等等），TeX 这一系列软件的发展全靠社区，而不像 InDesign、Illustrator、Word 这类软件，背后是 Adobe、微软等各个巨头的推动。

LaTeX 是简单的、优雅的，只要你不管底层的 TeX，但它不是 Word，使用请一定要好好看入门文档。
...
如果只是使用 LaTeX 不太建议看高德纳的 *The TeXbook*，因为这本书讲的是 TeX 而不是 LaTeX。
...
硬盘有两种：机械硬盘和固态硬盘 (SSD)。机械硬盘速度很慢，但容量可以很大，价格较便宜，适合用作数据盘。固态硬盘速度很快，大容量的 SSD 更是昂贵，个人建议购买 0.5 TB 或 256 GB 固态硬盘，用作系统盘并且在固态硬盘上面安装应用程序<sup><a id="footnote-ssd-ref" href="#footnote-ssd">[4]</a></sup>。
...
Dell 笔记本电脑售后服务较为贴心，购买后一年内可免费上门维修，免费维修和更换软件和硬件<sup><a id="footnote-dell_service-ref" href="#footnote-dell_service">[1]</a></sup>。
...
* Mac 端常用软件推荐	
	...
	* [MySQL](https://www.mysql.com/) 或 [PostgreSQL](https://postgresql.org)，数据库。Mac 下没有 SQL Server
